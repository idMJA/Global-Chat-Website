				...
				<div className="flex flex-wrap justify-center gap-12 max-w-6xl">
					{botsData.map((bot) => (
						<motion.div
							key={bot.id}
							variants={
								bot.cardVariant === "left" ? cardVariants : cardVariantsRight
							}
							transition={{ duration: 1, ease: "easeOut" }}
							whileHover={{
								scale: 1.1,
								rotate: bot.cardVariant === "left" ? 1 : -1,
								transition: { duration: 0.3 },
							}}
							whileTap={{ scale: 0.95 }}
						>
							<Card className="w-96 min-h-[460px] flex flex-col bg-white/5 backdrop-blur-xl border-white/10 hover:bg-white/10 transition-all duration-500 hover:shadow-2xl hover:shadow-white/10 group">
								<CardHeader className="text-center pb-6">
									<motion.div
										...
										whileHover={{
											rotate: bot.cardVariant === "left" ? 12 : -12,
											scale: 1.03,
										}}
